# AI Listing Search Specification

## Purpose

Defines public natural-language listing search through backend-owned GigaChat integration, including prompt validation, filter extraction, upstream error handling, and published-listing visibility.
...
- **WHEN** a client sends `POST /api/listings/ai-search` with `{"prompt":"Need a conference hall in Moscow for 30 people under 5000 per hour"}`
...
- **WHEN** the validated filter is `city=Moscow`, `spaceType=CONFERENCE_HALL`, `minCapacity=30`, and `maxPricePerHour=5000.00`
- **THEN** every returned listing is published
- **AND** every returned listing has city `Moscow` ignoring case
